...
if ( $_SERVER['REQUEST_METHOD'] === 'POST' && isset( $_POST['submit_claim'] ) && wp_verify_nonce( $_POST['claim_nonce'], 'submit_struttura_claim' ) ) {
    ...
    // Note field (always available)
    $claim_data['note'] = sanitize_textarea_field( $_POST['note'] ?? '' );
    ...
    <div class="container">
        <div class="claim-content">
            ...
            <form method="post" class="claim-form">
                ...
                <h2><?php esc_html_e( 'Note Aggiuntive', 'caniincasa' ); ?></h2>

                <div class="form-row">
                    <div class="form-group">
                        <label for="note"><?php esc_html_e( 'Note', 'caniincasa' ); ?></label>
                        <textarea id="note" name="note" rows="5" placeholder="<?php esc_attr_e( 'Aggiungi eventuali note o informazioni aggiuntive...', 'caniincasa' ); ?>"></textarea>
                        <small class="form-help-text"><?php esc_html_e( 'Questo campo è opzionale. Puoi usarlo per fornire informazioni aggiuntive o chiarimenti.', 'caniincasa' ); ?></small>
                    </div>
                </div>
