...
/**
 * Get custom login URL with optional redirect
 *
 * @param string $redirect_to URL to redirect to after login.
 * @return string Login URL.
 */
function caniincasa_login_url( $redirect_to = '' ) {
...
/**
 * Get custom registration URL with optional redirect
 *
 * @param string $redirect_to URL to redirect to after registration.
 * @return string Registration URL.
 */
function caniincasa_registration_url( $redirect_to = '' ) {
	$registration_url = home_url( '/registrazione/' );

	if ( $redirect_to ) {
		$registration_url = add_query_arg( 'redirect_to', urlencode( $redirect_to ), $registration_url );
	}
 ...
 */
function caniincasa_generate_verification_token( $user_id ) {
	$token = wp_generate_password( 32, false );
	$token_hash = wp_hash( $token );

	// Store hashed token with expiration (24 hours)
	update_user_meta( $user_id, 'email_verification_token', $token_hash );
	update_user_meta( $user_id, 'email_verification_token_expires', time() + ( 24 * HOUR_IN_SECONDS ) );
 ...
 */
function caniincasa_send_verification_email( $user_id, $token ) {
	...
	$verification_url = add_query_arg(
		...
		home_url( '/verifica-email/' )
	);

	$subject = 'Conferma la tua email - Caniincasa';
	...
	<head>
		<style>
			...
			.header { background-color: #ffcc70; padding: 30px; text-align: center; border-radius: 8px 8px 0 0; }
			.header h1 { margin: 0; color: #333; font-size: 28px; }
			.content { background-color: #f9f9f9; padding: 40px 30px; }
			.button { display: inline-block; padding: 15px 40px; background-color: #ffcc70; color: #333; text-decoration: none; border-radius: 5px; font-weight: bold; margin: 20px 0; }
			.footer { background-color: #333; color: #fff; padding: 20px; text-align: center; border-radius: 0 0 8px 8px; font-size: 14px; }
	...
	<body>
		<div class="container">
			...
			<div class="content">
				<h2>Ciao <?php echo esc_html( $user->first_name ); ?>!</h2>
				<p>Grazie per esserti registrato su Caniincasa.it!</p>
				<p>Per completare la registrazione e iniziare a pubblicare annunci, devi confermare il tuo indirizzo email cliccando sul pulsante qui sotto:</p>

				<div style="text-align: center;">
					<a href="<?php echo esc_url( $verification_url ); ?>" class="button">Conferma Email</a>
				</div>
				...
				<div class="warning">
					<strong>⚠️ Importante:</strong> I tuoi annunci non saranno approvati fino a quando non confermerai il tuo indirizzo email.
				</div>

				<p style="color: #999; font-size: 13px; margin-top: 30px;">
					Questo link è valido per 24 ore. Se non hai richiesto questa registrazione, puoi ignorare questa email.
				</p>
			</div>
			<div class="footer">
				<p>© <?php echo date( 'Y' ); ?> Caniincasa.it - Tutti i diritti riservati</p>
	...
	$headers = array(
		...
		'From: Caniincasa <noreply@caniincasa.it>',
	);
 ...
 */
function caniincasa_verify_email_token( $user_id, $token ) {
	$stored_hash = get_user_meta( $user_id, 'email_verification_token', true );
	$expires = get_user_meta( $user_id, 'email_verification_token_expires', true );

	if ( ! $stored_hash || ! $expires ) {
		return new WP_Error( 'invalid_token', 'Token di verifica non valido.' );
	}

	// Check if token expired
	if ( time() > $expires ) {
		return new WP_Error( 'expired_token', 'Il link di verifica è scaduto. Richiedi un nuovo link.' );
	}

	// Verify token
	if ( wp_hash( $token ) !== $stored_hash ) {
		return new WP_Error( 'invalid_token', 'Token di verifica non valido.' );
	}
 ...
 */
function caniincasa_handle_email_verification() {
	...
	if ( ! $user_id || ! $token ) {
		wp_die( 'Link di verifica non valido.', 'Errore Verifica Email', array( 'response' => 400 ) );
	}

	$result = caniincasa_verify_email_token( $user_id, $token );

	if ( is_wp_error( $result ) ) {
		wp_redirect( home_url( '/registrazione/?verification=failed&message=' . urlencode( $result->get_error_message() ) ) );
		exit;
	}
	...
	// Get redirect_to from user meta (saved during registration) or URL parameter
	$redirect_to = get_user_meta( $user_id, 'post_verification_redirect', true );
	if ( ! $redirect_to ) {
		$redirect_to = isset( $_GET['redirect_to'] ) ? esc_url_raw( $_GET['redirect_to'] ) : home_url( '/dashboard' );
	}

	// Clean up the meta after using it
	delete_user_meta( $user_id, 'post_verification_redirect' );

	wp_redirect( add_query_arg( 'verification', 'success', $redirect_to ) );
	exit;
}
add_action( 'template_redirect', 'caniincasa_handle_email_verification' );
...
function caniincasa_ajax_register_user() {
    ...
    // Create user with 'pending' role (will be changed after email verification)
    $user_data = array(
        'user_login'   => $username,
        'user_email'   => $email,
        'user_pass'    => $password,
        ...
        'role'         => 'subscriber', // Temporary role, will be changed after verification
    );
    ...
    // Save intended user type (will be applied after email verification)
    ...
    // Save redirect_to for after email verification
    if ( $redirect_to ) {
        update_user_meta( $user_id, 'post_verification_redirect', $redirect_to );
    }
    ...
    $token = caniincasa_generate_verification_token( $user_id );
    $email_sent = caniincasa_send_verification_email( $user_id, $token );

    if ( ! $email_sent ) {
        // If email fails, still proceed but log the error
        error_log( 'Failed to send verification email to user ID: ' . $user_id );
    }

    // DO NOT auto-login - user must verify email first

    wp_send_json_success( array(
        'message'      => 'Registrazione completata! Controlla la tua email per confermare l\'indirizzo e completare la registrazione.',
        'redirect_url' => home_url( '/registrazione/?registration=success' ),
