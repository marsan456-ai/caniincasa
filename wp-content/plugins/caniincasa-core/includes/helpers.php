...
function caniincasa_require_login() {
    if ( ! is_user_logged_in() ) {
        // Use custom login URL if function exists, otherwise fallback to home login
        $redirect_to = $_SERVER['REQUEST_URI'] ?? '';
        $login_url = function_exists( 'caniincasa_login_url' )
            ? caniincasa_login_url( $redirect_to )
            : add_query_arg( 'redirect_to', urlencode( $redirect_to ), home_url( '/login/' ) );
